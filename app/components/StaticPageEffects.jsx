...
export default function StaticPageEffects() {
  ...
  useEffect(() => {
    ...
    const activateProductItemPage = () => {
      ...
      setAttr("[data-product-inquiry-trigger]", "data-product-inquiry-title", item.title);
    };
    ...
    const openProductInquiryModal = (productName = "", productTitle = "") => {
      ...
      const detailsField = modal.querySelector('textarea[name="details"]');
      if (detailsField && productTitle) {
        const productLine = `Product: ${productTitle}`;
        if (!detailsField.value.includes(productLine)) {
    ...
    const handleProductInquiryTriggerClick = (event) => {
      ...
      const productTitle = trigger.dataset.productInquiryTitle || "";
      if (!openProductInquiryModal(productName, productTitle) && trigger.href) {
    ...
    const prefillInquiryFormsFromUrl = () => {
      ...
      const productTitle = params.get("item") || "";
      ...
      document.querySelectorAll("[data-inquiry-form]").forEach((form) => {
        ...
        if (productSelect && productName) {
          ...
          const matchingOption = [...productSelect.options].find((option) =>
            option.textContent.trim().toLowerCase() === normalizedProductName
          );
          if (matchingOption) productSelect.value = matchingOption.value || matchingOption.textContent;
        }
    ...
    const getProductPageFromUrl = () => Math.max(
      1,
      Number(new URLSearchParams(window.location.search).get("page")) || 1
    );
    const getProductPageHref = (page) => {
    ...
    const updateProductPageUrl = (page) => {
    ...
    const renderProductPagination = (pageCount) => {
      ...
      productPagination.innerHTML = Array.from({ length: pageCount }, (_, index) => {
        ...
        return `<a class="${isActive ? "active" : ""}" href="${getProductPageHref(page)}" data-product-page="${page}" aria-current="${isActive ? "page" : "false"}">${page}</a>`;
      }).join("");
    };
    ...
    const handleProductPaginationClick = (event) => {
      ...
      applyProductFilter(selectedProductCategory, false, nextPage);
      updateProductPageUrl(nextPage);
      document.querySelector(".products-all-section")?.scrollIntoView({ behavior: "smooth", block: "start" });
    };
    ...
    const initialProductFilter = [...productCategoryButtons].some((button) => button.dataset.productFilter === productHash)
      ? productHash
      : "all";
    applyProductFilter(initialProductFilter, false, getProductPageFromUrl());
    ...
    const renderCategoryPagination = (state, pageCount) => {
      ...
      state.pagination.innerHTML = Array.from({ length: pageCount }, (_, index) => {
        ...
        return `<a class="${isActive ? "active" : ""}" href="${getProductPageHref(page)}" data-product-grid="${state.id}" data-product-category-page="${page}" aria-current="${isActive ? "page" : "false"}">${page}</a>`;
      }).join("");
    };
    ...
    const handleCategoryPaginationClick = (event) => {
      ...
      const nextPage = Number(button.dataset.productCategoryPage) || 1;
      applyCategoryPagination(state, nextPage, true);
      updateProductPageUrl(nextPage);
    };

    categoryPaginationStates.forEach((state) => {
      const section = state.grid.closest(".product-type-section");
      applyCategoryPagination(state, section?.hidden ? 1 : getProductPageFromUrl());
    });
    const handleProductPagePopState = () => {
      const page = getProductPageFromUrl();
      applyProductFilter(selectedProductCategory, false, page);
      categoryPaginationStates.forEach((state) => {
        const section = state.grid.closest(".product-type-section");
        applyCategoryPagination(state, section?.hidden ? 1 : page);
      });
    };
    document.addEventListener("click", handleCategoryPaginationClick);
    window.addEventListener("popstate", handleProductPagePopState);
    ...
    return () => {
      ...
      window.removeEventListener("popstate", handleProductPagePopState);
